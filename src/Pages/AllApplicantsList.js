...
export default function AllApplicantsList() {
  ...
  const {
    ApplicantsList, EmpViewAllApplicants,
  ...
  useEffect(() => {
    (async () => {
      if (!token) {
        ...
        try {
          await EmpViewAllApplicants(token);
  ...
  const filterCategory = async (e) => {
    ...
    if (value === '--filter jobs---') {
      ...
      const res = await (await fetch(`${url}/api/user/filterJobsApplications?Title=${value}`, config)).json();
      if (res.status === 200) {
        setLoading(false);
        seeSetTitles(true);
          ...
          {loading ? <p>Loading....</p> :
            seeTiltes ? (
              <table id="customers">
                ...
                <tbody>
                  {state.jobLists.applications.length === 0 ? <tr style={{ padding: "10px", textAlign: "left" }}>
                    <td>No Applicants found for this job</td>
                  </tr>
                    : state.jobLists.applications.map(({ id, applicant_name, email, job_title, date }) => {
                      return (
                        <tr key={id}>
              ...
              <table id="customers">
                ...
                <tbody>
                  {ApplicantsList.applicants.length === 0 ? <tr style={{ padding: "10px", textAlign: "left" }}>
                    <td> No Applicants yet </td>
                  </tr>
                    : ApplicantsList.applicants.map(({ id, applicant_name, email, job_title, date }) => {
                      return (
                        <tr key={id}>
                          <td>{applicant_name}</td>
                          <td>{email}</td>
